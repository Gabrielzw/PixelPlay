...
- 将本地视频和远程 WebDAV 视频放到同一个应用中处理，减少在不同播放器和文件管理器之间切换。
- 对移动端观影场景做了较多实际交互支持，例如续播、历史记录、收藏、播放列表和播放器默认行为配置。
- 项目结构清晰，按功能模块拆分，适合作为 Flutter + GetX + Isar + Android 原生桥接的中型示例工程。
...
仓库当前未包含单独的项目源码许可证文件。如果你计划对本项目进行分发、复用或二次发布，建议先与维护者确认授权方式。
